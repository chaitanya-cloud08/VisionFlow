...
export default function Dashboard() {
  ...
  const handleGoToFocus = () => {
    navigate('/focus'); // Navigate to the dashboard route
  };
  ...
  // Fetch background image from localStorage on component mount
  useEffect(() => {
    ...
    <div className='backdrop-blur-3xl bg-gradient-to-r from-violet-400 to-pink-300 flex flex-col h-screen border-none shadow-lg overflow-hidden' style={{ backgroundImage: bgImage, backgroundSize: 'cover', backgroundPosition: 'center' }} > 
      ...
      <div className='flex-grow flex flex-col md:flex-row '>
        ...
        <div className="blur-container backdrop-blur-sm p-4 rounded-3xl ">
          ...
          <div className='greet'>
            <h1 className=' text-3xl md:text-3xl text-white font-bold text-center'>{greeting}</h1>
          </div>
          <div className='clk'>
            <h1 className=' text-5xl md:text-9xl text-white font-bold text-center'>{currentTime}</h1>
          </div>
        </div>
        </div>
